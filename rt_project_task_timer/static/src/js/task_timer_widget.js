...
export class TaskTimerWidget extends Component {
    ...
    setup() {
        ...
        useEffect(
            ...
            () => [
                this.props.record?.data?.timer_running,
                this.props.record?.data?.task_start_date,
                this.props.record?.data?.task_accumulated_time,
            ]
        );
    }
    ...
    syncWithRecord() {
        ...
        const accumulated = record.task_accumulated_time || 0;
        ...
        if (record.timer_running && record.task_start_date) {
            ...
            this.baseSeconds = accumulated + (isNaN(elapsed) ? 0 : elapsed);
...
//        if (!record) return;
//
//        // 🔥 Only reset when record changed OR timer state changed
...
////
////        onMounted(() => {
////            this.initialize(this.props.record.data);
////        });
////
////        onWillUpdateProps((nextProps) => {
////            this.initialize(nextProps.record.data);
////        });
////
////        onWillUnmount(() => {
////            this.stopInterval();
...
////            const elapsed = Math.floor((now - start) / 1000);
////
////            this.baseSeconds = baseSeconds + (isNaN(elapsed) ? 0 : elapsed);
////
////            this.updateDisplay(this.baseSeconds);
////
////            this.interval = setInterval(() => {
...
////            if (!record) return;
////
////            // If same record, do nothing
////            if (this.currentResId === resId) return;
////
////            this.currentResId = resId;
////
////            this.stopInterval();
